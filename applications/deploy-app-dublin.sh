...
echo "🚀 Deploying Dad Jokes Application to Dublin (DR)..."
echo "================================================"

# Check if kubectl is configured
if ! kubectl cluster-info &> /dev/null; then
    echo "❌ kubectl is not configured. Please configure access to Dublin cluster first."
    exit 1
fi
...
# Deploy with Dublin-specific configuration
echo "🚀 Deploying application with DevSpace..."
REGION=eu-west-1 devspace deploy -n dev
...
echo ""
echo "✅ Dad Jokes application deployed to Dublin!"
echo ""
...
echo ""
echo "⚠️  Note: Ratings storage in Dublin:"
echo "    - Will be EMPTY on initial deployment (no ratings data)"
echo "    - Can be restored from Milan snapshot during DR scenario"
echo "    - Any new ratings in Dublin won't sync back to Milan"
echo ""
